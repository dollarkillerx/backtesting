...
#property script_show_inputs // 打开inputs
            ...
            if(PositionGetTicket(i) == ticket)
              {
               ...
               if(!PositionSelect(ticket))
                 {
                  Print("Error selecting order with ticket: ", ticket, ", Error: ", GetLastError());
                  return false;
                 }
...
// 获取货币符号信息并打印
   SymbolInfoPayload symbolInfo = EasyUtils::GetSymbolInfo("LTCUSDm");
...
// 示例下单
// EasyUtils::buy(symbolInfo.symbol, 0.1,10,10);
// EasyUtils::sell(symbolInfo.symbol, 0.1,10,10);

// EasyUtils::CloseAllBuy(symbolInfo.symbol);
// EasyUtils::CloseOrderByTicket(270356067);
//   EasyUtils::buy(symbolInfo.symbol, 0.1,10,10);
//   EasyUtils::buy(symbolInfo.symbol, 0.1,10,10);
//   EasyUtils::sell(symbolInfo.symbol, 0.1,10,10);
//   EasyUtils::sell(symbolInfo.symbol, 0.1,10,10);
   EasyUtils::CloseAll(symbolInfo.symbol);


// 使用 SymbolInfo 获取 iMA 数据
   SymbolInfo info;
   info.ma(symbolInfo.symbol, symbolInfo.period, 14, 0, MODE_SMA, PRICE_CLOSE);
