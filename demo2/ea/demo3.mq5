...
// 0. 获得最新的order记录
   ...
   if(PositionsTotal() != 0)
     {
      Print("rj:",last_position.last_position.ticket);
      Print("rj:",last_position.last_position.symbol);
      Print("rj:",last_position.last_position.time_msc);
      ...
      if(MinutesToMilliseconds(interval_time) > serverTime * 1000 - last_position.last_position.time_msc)
        {
         Print("la");
         // Print("serverTime",serverTime * 1000 - last_position.last_position.time_msc);
         return;
        }
      // Print("MinutesToMilliseconds: ", MinutesToMilliseconds(interval_time));
      // Print("serverTimeMillis: ", serverTime * 1000);
      // Print("MinutesToMilliseconds2: ", serverTime * 1000 - last_position.last_position.time_msc);
      // Print("MinutesToMilliseconds3: ", MinutesToMilliseconds(interval_time) < serverTime * 1000 - last_position.last_position.time_msc);
      // 价格间隔

      // if (MathAbs(SymbolInfoDouble(symbol, SYMBOL_ASK) - last_position.last_position.price_open)) {
