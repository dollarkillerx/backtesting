...
// 价格间隔
   ...
   if(AccountInfoDouble(ACCOUNT_PROFIT) > 0.5)
     {
      return;
     }
   ...
   if(last_position.last_position.direction == POSITION_TYPE_BUY)
     {
      easyUtils.Buy(symbol, new_volume * 10,15);
     }
   else
     {
      easyUtils.Sell(symbol, new_volume * 10,15);
     }
...
// 如果都盈利且时间> 1h 就关闭
   if(profitable_quantity == PositionsTotal())
     {
